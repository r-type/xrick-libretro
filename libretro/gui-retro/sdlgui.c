...
extern unsigned short int Retro_Screen[1024*1024];
...
#define Draw_text SDL_Draw_text
#define DrawFBoxBmp SDL_DrawFRect
#define input_gui() Retro_PollEvent()
/*
...
void draw_cross(int x,int y) {
	...
	for(j=y;j<y+dy;j++){
		idx=0;
		for(i=x;i<x+dx;i++){
			if(cross[j-y][idx]=='.')DrawPointBmp(Retro_Screen,i,j,0xffff);
...
*/
	Draw_text(sdlscrn,x,y,1,0,1,2,40,(char *)txt);
}
